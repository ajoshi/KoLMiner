...
class OutfitManager {
  ...
  Future<bool> equipPreparedOutfit(int? outfitId) async {
    ...
    var response = await _network.makeRequestWithQueryParams("inv_equip.php",
        "action=outfit&which=2&whichoutfit=$outfitId",
  ...
  Future<bool> equipSinglePiece(int equipmentId) async {
    var response = await _network.makeRequestWithQueryParams("inv_equip.php",
        "which=2&action=equip&whichitem=$equipmentId",
        method: HttpMethod.POST,
        allowEmptyResponse: true);
    return (response.responseCode == NetworkResponseCode.SUCCESS);
  }
  ...
  Future<bool> equipOutfitUsingName(String outfitName) async {
    var response = await _network.makeRequestWithQueryParams("submitnewchat.php",
